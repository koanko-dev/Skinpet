  ...
  return (
    <ResultStageContent>
      ...
      {jsonResult && (
        <ResultBox>
          <h3>
            이미지 분석 결과 발견된 피부질환은 <span>'{diseaseNameConverter(jsonResult.className)}'</span> 입니다.
          </h3>
          <p>피부질환에 대한 더 정확한 진단을 위해서는 전문의사의 상담을 권장드립니다.</p>
          <span>
            *만약 피부 질환 발생 부위가 아닌 다른 부분이 측정됐다면, 피부 질환
            부위가 잘 보이는 다른 이미지를 넣어주세요
          </span>
          <Button theme="outline" onClick={onClickPrevStage}>다른 이미지로 업로드하기</Button>
        </ResultBox>
      )}
    </ResultStageContent>
  );
